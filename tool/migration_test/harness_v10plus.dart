// Migration-test harness overlay for v10.1+/v11.x versions (storageNamespace
// available). See harness_v9.dart for the pre-storageNamespace variant and
// tool/migration_test/run.sh for how these get built and run.
...
void main() async {
  ...
  runApp(const MaterialApp(
    home: Scaffold(body: Center(child: Text('migtest harness (v10+)'))),
  ));

  final profiles = <String, FlutterSecureStorage>{
    'default': const FlutterSecureStorage(),
    // Same logical store as harness_v9.dart's namespace_migtest profile, now
    // following the documented sharedPreferencesName -> storageNamespace
    // migration (same value, new option name).
    'namespace_migtest': const FlutterSecureStorage(
      aOptions: AndroidOptions(storageNamespace: 'migtest_ns'),
    ),
  };
